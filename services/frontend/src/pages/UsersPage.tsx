    ...
    <div className="p-6 space-y-6 bg-black min-h-screen">
      ...
      <div className="bg-[#050505] rounded-xl border border-primary/30 overflow-hidden">
        <table className="w-full">
          ...
          <tbody>
            {users.map((user) => (
              <tr key={user.id} className="border-b border-primary/20 hover:bg-[#0a0a0a] transition-all duration-150">
                ...
                <td className="px-5 py-4">
                  <select
                    ...
                    className="bg-[#0a0a0a] border border-warning/30 text-white text-xs rounded-lg px-3 py-1.5 focus:outline-none focus:border-primary disabled:opacity-50 transition-all duration-150"
                  >
                    {ROLES.map(role => (
