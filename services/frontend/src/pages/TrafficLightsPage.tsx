      ...
      <div className="flex justify-between items-start mb-3">
        <div>
          <div className="flex items-center gap-1.5">
            <p className="text-[11px] font-mono text-warning">{info.lampId}</p>
            <span className={`text-[7px] px-1.5 py-0.5 rounded-full font-bold ${
              info.intersectionType === 'semi_smart' ? 'bg-warning/20 text-warning' : 'bg-slate-700/50 text-slate-500'
        ...
        <button onClick={onClose} className="text-slate-600 hover:text-white text-xs ml-3 shrink-0">Kapat</button>
      </div>
      ...
      <div className="space-y-1.5">
        ...
        {completedHistory.map((h, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            <span className="ml-auto text-[8px] font-mono text-warning/80">{h.duration}</span>
          </div>
        ))}
      </div>
    </div>
